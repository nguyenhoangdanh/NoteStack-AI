...
import { useNotes, useWorkspaces, useDefaultWorkspace, useCreateNote, useDeleteNote } from '../lib/query';
...
export default function Sidebar({ className }: SidebarProps) {
  ...
  if (!sidebarOpen) {
    return (
      <div className={cn("w-12 border-r bg-background flex flex-col items-center py-4 gap-2", className)}>
        <Button
          variant="ghost"
          size="sm"
        ...
        <div className="p-4 border-b">
          ...
          <div className="space-y-1">
            {workspaces.map((workspace) => (
              <Button
                key={workspace._id}
                variant={selectedWorkspaceId === workspace._id ? "secondary" : "ghost"}
                size="sm"
