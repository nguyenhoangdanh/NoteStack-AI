...
export function parseMarkdownFile(content: string, filename: string): ImportedFile {
  ...
  if (frontMatterMatch) {
    ...
    if (tagsMatch) {
      ...
      tags.push(...tagString.split(',').map(tag => tag.trim().replace(/['"]/g, '')));
    }
...
export async function importMultipleFiles(files: FileList): Promise<ImportedFile[]> {
...
export function exportAllNotes(notes: Array<{
  title: string;
  content: string;
  tags: string[];
  createdAt: number;
