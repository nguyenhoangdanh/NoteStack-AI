...
export default function CollaborationPage() {
  ...
  const { data: shareStats, isLoading: shareStatsLoading } =
    useUserShareStats();
  ...
  const handleInviteCollaborator = async () => {
    ...
    try {
      await inviteCollaborator.mutateAsync({
        ...
        data: {
          email: inviteEmail,
          permission: invitePermission.toLowerCase() as
            | "read"
            | "write"
            | "admin",
        },
          ...
          <Card className="card-gradient">
            <CardContent className="p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold">
                    {finalShareStats.activeCollaborations}
                  </p>
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <Badge variant="outline">
                {filteredCollaborations.length} collaboration
                {filteredCollaborations.length !== 1 ? "s" : ""}
              </Badge>
            </div>
          </CardContent>
        </Card>
                  ...
                  <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4 text-xs text-muted-foreground">
                          <span className="flex items-center gap-1">
                            ...
                            {collaboration.lastActive
                              ? formatDistanceToNow(
                                  new Date(collaboration.lastActive),
                                  { addSuffix: true },
                                )
                              : "Never"}
                          </span>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-muted-foreground mb-2">
                        Updated{" "}
                        {collaboration.note?.updatedAt
                          ? formatDistanceToNow(
                              new Date(collaboration.note.updatedAt),
                              { addSuffix: true },
                            )
                          : "Unknown"}
                      </p>
