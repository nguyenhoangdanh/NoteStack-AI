    ...
    if (trimmedLine.startsWith("#")) {
      ...
      if (estimateTokens(currentChunk) > options.maxTokens) {
        ...
        if (sentences.length > 1) {
          ...
          // Add overlap
          const overlapWords = firstPart
            .split(" ")
            .slice(-options.overlap)
            .join(" ");
  ...
  while (selected.length < k && remaining.length > 0) {
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      const maxSimilarityToSelected = Math.max(
        ...selected.map((sel) =>
          cosineSimilarity(candidate.embedding, sel.embedding),
        ),
      );

      // MMR score: λ * sim(query, doc) - (1-λ) * max(sim(doc, selected))
      const mmrScore =
        lambda * candidate.similarity - (1 - lambda) * maxSimilarityToSelected;
